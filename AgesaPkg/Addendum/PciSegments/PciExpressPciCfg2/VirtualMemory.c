...
/**
  ...
  reserve at least another PAGE_TABLE_POOL_UNIT_PAGES. But usually this will not
  happen in practice.
  ...
  )
{
  ...
  Buffer = (VOID*) (UINTN) PcdGet32 (PcdAmdPeiTempPageTableBaseAddress);
  if (Buffer == NULL) {
    ...
    if (PoolUnitSize >= LevelSize[Level]) {
      ...
      if ((PageAttr & IA32_PG_RW) != 0) {
        while (PoolUnitSize > 0) {
          ...
          // one page (2MB). Then we do not need to update attributes for pages
